...
- Keep the harness independent of CX Autopilot and all other consuming products.
- Keep `ai-assistant-harness` intact as a separate project.
- Treat this project as a code and architecture fork, not a literal GitHub fork.
- Reuse proven ideas from `ai-assistant-harness` only when they still fit the broader enterprise runtime.
- Remove conversational and read-only assumptions unless they are intentional and documented.
...
## Assistant Harness Reuse

`ai-assistant-harness` is the source of proven patterns, not a dependency target.

For each extracted module or concept, classify it as one of:
...
Preserve the migration decision in project documentation.

Use these default migration directions:

- `providers.py` -> generalize into `providers/`
- `tools.py` -> generalize for read, write, and action tools
- `permission.py` -> reuse and generalize into governance permissions
- `loops.py` -> generalize into runtime execution
- `audit.py` -> reuse and generalize into observability audit
- `rules.py` -> generalize into governance safety
- `context.py` -> reuse trusted and untrusted context concepts
- `state.py` -> generalize into state storage
- `memory.py` -> redesign as optional memory strategies
- `skills.py` -> evolve into capability contracts
- `verification.py` -> generalize beyond citations and response sections
- `recovery.py` -> redesign around enterprise agent outcomes
- `harness.py` -> redesign around general enterprise execution
- `evaluation_contracts.py` -> reuse only runtime-facing trace and replay contracts
- `evals.py` -> do not port full evaluation ownership into this project

Do not copy code only because it already exists. Port behavior only when the new runtime still needs it.
...
The registry must support lifecycle state such as active, suspended, deprecated, and retired.
...
When porting behavior from `ai-assistant-harness`, make the source and migration intent clear in the commit or project documentation.
